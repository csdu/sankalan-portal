...
<div class="flex flex-col justify-center flex-1">
    <div class="container mx-auto px-4 flex flex-col items-center lg:flex-row flex-1">
        <div class="my-6 lg flex flex-col justify-center items-start flex-1 text-center lg:text-left">
            <h1 class="mb-1 text-4xl">Sankalan <span class="text-2xl text-blue-600">{{ config('app.sankalan_start_time')->format('Y') }}</span></h1>
                ...
                <countdown-timer :duration="{{ $timeLeft }}" v-slot="{timer, format}">
                    <div class="flex text-3xl sm:text-5xl -mx-4 mb-4">
                        <span v-if="timer.days > 0" class="inline-flex flex-col items-center justify-center p-3 md:p-6">
                            ...
                            <span class="mt-1 text-xs uppercase text-slate-600 tracking-wide font-semibold">Days</span>
                        </span>
                        <span class="inline-flex flex-col items-center justify-center p-3 md:p-6">
                            <span v-text="format(timer.hours, 2)"></span>
                            <span class="mt-1 text-xs uppercase text-slate-600 tracking-wide font-semibold">Hours</span>
                        </span>
                        <span class="inline-flex flex-col items-center justify-center p-3 md:p-6">
                            <span v-text="format(timer.minutes, 2)"></span>
                            <span class="mt-1 text-xs uppercase text-slate-600 tracking-wide font-semibold">Minutes</span>
                        </span>
                        <span class="inline-flex flex-col items-center justify-center p-3 md:p-6">
                            <span v-text="format(timer.seconds, 2)"></span>
                            <span class="mt-1 text-xs uppercase text-slate-600 tracking-wide font-semibold">Seconds</span>
                        </span>
                    </div>
                </countdown-timer>
            @endif
            <div class="inline-block text-left px-4 py-2 bg-blue-50 text-blue-800 border border-blue-600 rounded-md min-w-1/2">
                ...
                <transition name="fade" mode="out-in">
                    <div v-if="isRegister" class="w-full card my-4">
                        <h2 class="card-header">
                            ...
                            <a class="text-sm uppercase text-blue-500 tracking-wide" href="#login" @click="login">Login</a>
                        </h2>
                        <form method="POST" action="{{ route('register') }}" class="card-content">
                            @csrf
                            <div class="flex mb-3">
                                <div class="flex-1 mr-2">
                                    <input type="text" placeholder="First Name" class="control{{ $errors->has('first_name') ? ' border-red-500' : '' }}" name="first_name" value="{{ old('first_name') }}" required autofocus>
                    ...
                    <div {{ $timeLeft > 0 ? 'v-else' : '' }} class="w-full card my-4">
                        <h2 class="card-header">
                            {{ __('Login') }}
                            <span class="text-xs text-slate-600 mx-1">or</span>
                            <a class="text-sm uppercase text-blue-500 tracking-wide" href="#register" @click="register">Register</a>
                        </h2>
