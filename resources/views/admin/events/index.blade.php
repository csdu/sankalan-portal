            ...
            @foreach($events as $event)
                <tr class="border-t hover:bg-grey-lighter">
                    <td class="table-fit text-left pl-6 py-2">
                        @if ($event->is_live)
                            <span class="p-1 uppercase text-xs rounded bg-green text-white">Live</span>
                        @elseif($event->ended_at)
                            <span class="p-1 uppercase text-xs rounded bg-blue text-white">Ended</span>
                        @else
                            <span class="p-1 uppercase text-xs rounded bg-grey-light">not started</span>
                        @endif
                    </td>
                    ...
                    <td class="table-fit text-center px-4 py-2">
                        @if($event->rounds > 1)
                            <span class="px-2 py-1 rounded-full bg-grey text-xs">{{ $event->rounds }}</span>
                        @else
                            <span class="px-2 py-1 bg-blue text-white font-extralight rounded text-xs">No Prelims</span>
                        @endif
                    </td>
                    <td class="table-fit text-center px-4 py-2">
                        ...
                        @else
                            <span class="text-xs text-red">No Participation</span>
                        @endif
                    </td>
                    <td class="table-fit text-center px-4 py-2">
                        @if ($event->quizzes_count)
                            <span class="px-2 py-1 rounded-full bg-grey text-xs">{{ $event->quizzes_count }}</span>
                        @else
                            <span class="text-xs text-red">No Quiz</span>
                        @endif
                    </td>
